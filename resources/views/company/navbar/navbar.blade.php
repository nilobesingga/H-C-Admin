...
@if($parentModule && $childModules->isNotEmpty())
    <div class="bg-[--tw-header-bg] dark:bg-[--tw-header-bg-dark] border-b border-b-neutral-200 header-navigation w-full">
        <div class="flex flex-wrap items-center justify-between gap-2 pl-3 pr-2 container-fluid">
            <div class="grid">
                <div class="scrollable-x-auto">
                    <div class="gap-5 menu lg:gap-10" data-menu="true">
                        @foreach($childModules as $module)
                            <div class="menu-item py-2 border-b-2 border-b-transparent hover:border-b-neutral-900 transition-all duration-300 {{ $isChildActive($module) ? '!border-b-brand-active' : '' }}">
                                <a class="menu-link gap-2.5" href="{{ route($module->route_name) }}">
                                    <span class="text-sm font-medium tracking-tight text-black menu-title text-nowrap">
                                        {{ $module->name }}
                                    </span>
                                </a>
                            </div>
