                ...
                <table class="table" style="margin-top:2rem">
                    ...
                    <tbody>
                        @foreach($protectors as $protector)
                            <tr>
                                <td class="text-center"> @php echo date("d M Y", strtotime($protector['startDate'] )); @endphp</td>
                                <td>{{isset($protector['name']) ? $protector['name']: '' }}</td>
                                <td>{{isset($protector['dateOfBirth']) ? $protector['dateOfBirth'] : ''}} <br>
                                    {{isset($protector['nationality']) ? $protector['nationality']: ''}}
                                </td>
                                <td>{{isset($protector['address']) ? $protector['address']: '' }}</td>
                                <td>{{isset($protector['notes']) ? $protector['notes']: '' }}</td>
                                <td class="text-center">
                                    @php
                                        if($protector['endDate'] != null)
                                        echo date("d M Y", strtotime($protector['endDate'] ));
                                    @endphp
                                </td>
