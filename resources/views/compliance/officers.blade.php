                ...
                <table class="table" style="margin-top:2rem">
                    ...
                    <tbody>
                        @foreach($officers as $officer)
                            <tr>
                                <td class="text-center"> @php echo date("d M Y", strtotime($officer['startDate'] )); @endphp</td>
                                <td>{{isset($officer['name']) ? $officer['name']: '' }}</td>
                                <td>{{isset($officer['dateOfBirth']) ? $officer['dateOfBirth'] : ''}} <br>
                                    {{isset($officer['nationality']) ? $officer['nationality']: ''}}
                                </td>
                                <td>{{isset($officer['address']) ? $officer['address']: '' }}</td>
                                <td>{{isset($officer['notes']) ? $officer['notes']: '' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
