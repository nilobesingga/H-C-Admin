...
<main>
    ...
    <div  class="container default-font" >
            <x-compliance_header
                ...
                :authority="$authority"
                :title="'Register of Beneficiaries'"
                ...
                <table class="table" style="margin-top:2rem">
                    ...
                    <tbody>
                        @foreach($beneficiaries as $beneficiary)
                            <tr>
                                <td class="text-center"> @php echo date("d M Y", strtotime($beneficiary['startDate'] )); @endphp</td>
                                <td>{{isset($beneficiary['name']) ? $beneficiary['name']: '' }}</td>
                                <td>{{isset($beneficiary['dateOfBirth']) ? $beneficiary['dateOfBirth'] : ''}} <br>
                                    {{isset($beneficiary['nationality']) ? $beneficiary['nationality']: ''}}
                                </td>
                                <td>{{isset($beneficiary['address']) ? $beneficiary['address']: '' }}</td>
                                <td>{{isset($beneficiary['notes']) ? $beneficiary['notes']: '' }}</td>
                                <td class="text-center">
                                    @php
                                        if($beneficiary['endDate'] != null)
                                        echo date("d M Y", strtotime($beneficiary['endDate'] ));
                                    @endphp
                                </td>
