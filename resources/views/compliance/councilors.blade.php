                ...
                <table class="table" style="margin-top:2rem">
                    ...
                    <tbody>
                        @foreach($councilors as $councilor)
                            <tr>
                                <td class="text-center"> @php echo date("d M Y", strtotime($councilor['startDate'] )); @endphp</td>
                                <td>{{isset($councilor['name']) ? $councilor['name']: '' }}</td>
                                <td>{{isset($councilor['dateOfBirth']) ? $councilor['dateOfBirth'] : ''}} <br>
                                    {{isset($councilor['nationality']) ? $councilor['nationality']: ''}}
                                </td>
                                <td>{{isset($councilor['address']) ? $councilor['address']: '' }}</td>
                                <td>{{isset($councilor['role']) ? $councilor['role']: '' }}</td>
                                <td>{{isset($councilor['notes']) ? $councilor['notes']: '' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
